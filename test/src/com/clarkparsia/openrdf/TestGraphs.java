...
public class TestGraphs {
	...
	public void testList() throws Exception {
		...
		assertEquals(aElems, Graphs.asList(aGraph, aGraph.match(null, RDF.FIRST, aElems.get(0)).next().getSubject()));

		for (Resource aRes : aElems) {
			assertTrue(Graphs.isList(aGraph, aGraph.match(null, RDF.FIRST, aRes).next().getSubject()));
		}
	...
	public void testFilter() {
		...
		Statement st1 = aInput.getValueFactory().createStatement(s, RDF.TYPE, o);
		Statement st2 = aInput.getValueFactory().createStatement(s2, RDF.TYPE, o2);
	...
	public void testTransform() {
		...
		Function<Statement, Statement> xform = new Function<Statement, Statement>() {
			...
			public Statement apply(final Statement theStatement) {
				return aInput.getValueFactory().createStatement(theStatement.getSubject(), RDF.TYPE, RDFS.CLASS);
			}
		};
	...
	public void testAny() {
		...
		aInput.add(aInput.getValueFactory().createBNode(), RDF.TYPE, aInput.getValueFactory().createBNode());
	...
	public void testAll() {
		...
		Function<Statement, Statement> xform = new Function<Statement, Statement>() {
			@Override
			public Statement apply(final Statement theStatement) {
				return aInput.getValueFactory().createStatement(theStatement.getSubject(), RDF.TYPE, theStatement.getObject());
			}
		};
	...
	public void testFind() {
		...
		Statement st = aInput.getValueFactory().createStatement(aInput.getValueFactory().createBNode(), RDF.TYPE, aInput.getValueFactory().createBNode());
